...
test("React Native native integration registers appmodules and gates JSI delivery on lifecycle validity", () => {
  ...
  assert.match(podspec, /:execution_position => :after_compile/);
  ...
  assert.match(podspec, /NativeSymbolNemWalletCoreProvider\.\{h,mm\}/);
  ...
  assert.match(podspec, /libSymbolNemWalletCoreRN\.a/);
  ...
  assert.match(releaseProducer, /OTHER_LDFLAGS=\$\(inherited\) -force_load/);
  ...
  assert.match(releaseProducer, /SNWC_RN_ARTIFACT_MODE=1/);
  assert.match(releaseProducer, /OTHER_CPLUSPLUSFLAGS=\$\(inherited\)/);
