...
`docs/requirements/requirements.md` §6 冒頭に残る「独立した承認記録は本リポジトリに提供されていないため、これを承認記録そのものとは扱わない」という記述は、本 `DEC-REQ-001` の追加以前の状態を示すものであり、本決定の承認をもって失効する。

以後、第三者が承認状態を確認する場合は、本記録に記載した対象ファイル、commit SHA、blob SHAおよびGit履歴を正とする。要件本文に残る当該旧記述を、現時点の承認状態の根拠として使用しない。
...
`docs/requirements/requirements.md` の「ユーザー確定事項」および「要件定義更新依頼」に由来する確定事項は、本記録を介して承認済みベースラインへ追跡できる。これにより、RR-005の修正完了条件である第三者による承認済み決定記録への追跡性を満たすものとする。