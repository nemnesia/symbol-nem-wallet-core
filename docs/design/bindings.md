...
本書は、Rust Wallet Core を Desktop / React Native Android / React Native iOS / Web / Node.js の Application へ接続する Native C ABI、Node-API、Web / WASM、React Native Binding について、責務、依存方向、信頼境界、所有権、ライフサイクルの配置を定める。Web には Web Application と Browser Extension を含む。React Native は Mobile の具体的な v1 実行環境である。
...
対象は Native C ABI、Node-API、Web / WASM、React Native Binding に共通する設計責任と、React Native の Android / iOS 接続方式、package 内 runtime 分離、Native 成果物、buffer、error、failure policy である。特定の crate、ABI、JavaScript 型、package exports JSON、storage API、Browser context、memory technique は本書で決定しない。
...
Bindings Design の規範となる上流資料は次のとおりである。
...
Requirements
  ↓
Design（Architecture / Security Design / Bindings Design）
  ↓
Specification
  ↓
Implementation
...
- **Binding**: Application と Rust Wallet Core の間で、値、表現、所有権、ライフサイクル、エラー、警告を橋渡しする境界層。Core のセキュリティ上の権限は持たない。React Native では TypeScript facade、private runtime entry、JSI / TurboModule adapter、Android / iOS Native 層を含む。
...
### 3.2 Native / Node.js / Web / React Native 共通の保証境界

Native C ABI、Node-API、Desktop、Mobile、Web、WASM の方式差は、実行環境との表現、所有権、ライフサイクルの橋渡しに限定する。次の不変条件は全経路で共通とし、Native C ABI または Node-API を Web / WASM より強い秘密情報の隔離境界として扱わない。
...
- Native C ABI / Node-API / Web WASM / React Native の経路差によって、Core のセキュリティ上の意味、秘密情報の方針、認可、失敗時の方針が変わらないことの維持
...
Native C ABI、Node-API、Web / WASM、React Native の representation 差は、Core の compatibility、signing authority、secret ownership、authorization または failure meaning を変更しない。具体的な identifier、byte 表現、address、derivation、protocol、interop contract は下流へ委譲する。

## 8. Native 境界の安全側責任と保証の限界

### 8.1 Binding に求める設計上の役割
...
この責任により、Binding は検証できる不正入力をそのまま Core へ渡さず、失敗を成功に変換せず、Core の結果・エラー・警告の意味を置き換えない。失敗経路で秘密情報の出力や保持、永続的な秘密状態、部分状態を増やさず、既存の確定状態を壊さない。この方針は Native C ABI だけの緩和ではなく、Native C ABI / Node-API / Web WASM / React Native に共通するセキュリティ不変条件と整合する。
...
- **影響**: Native C ABI / Node-API / Web WASM / React Native の具体方式が変わっても、Core の所有責任、操作単位の認可、非開示、失敗時の安全性、互換性方針を維持できる。
- **見直し条件**: Core と Binding の責任分担を変更する上位 Requirements または Architecture が承認された場合。

### 9.2 Native / Node.js / Web / React Native 共通の保証境界

- **判断**: Native C ABI、Node-API、Web / WASM、React Native を同一の security invariant、host compromise limitation、non-authority boundary で扱う。Native C ABI または Node-API を Web より強い秘密隔離境界としない。
- **根拠**: Concept / Requirements の全環境共通原則、Architecture の全環境 trust boundary および Security Design の guarantee boundary。
- **代替案**: Web だけに host environment 侵害時の制約を置く方式は、Native / Mobile / Desktop の責任を曖昧にするため採用しない。
...
### 9.3 Store の不透明な仲介、現在の Store を選ぶ責任、v1 の移行非対応
...
| React Native architecture、runtime resolution、artifact、buffer、error、sync resource evidence、support matrix、verification | Requirements FR-019、NFR-006〜NFR-015、AC-051〜AC-061、UF-RN-001、DR-RN-001〜DR-RN-004 | §12.1〜§12.15 | §12.1〜§12.15 | §12.1〜§12.17 |
...
React Native New Architecture の current direction は、公式資料上も JSI、Turbo Native Module および Codegen を中心にしている。現行 product policy は [`react-native-platform-baseline.md`](../decisions/react-native-platform-baseline.md) の `PD-RN-001` と `PD-RN-006 = APPROVED` により、stable RN `0.86.x / 0.87.x` の finite support window と New Architecture mandatory を採用し、Legacy Architecture / Bridge を formal support に含めない。具体的な integration API は Specification へ委譲する。将来 Legacy support を追加する場合は、同期契約、lifecycle、security および追加 CI / release evidence を含む新しい正式判断を必要とする。

### 12.3 公開 API の基準と同期・非同期方針
...
iOS の approved support matrix は `PD-RN-003` / `PD-RN-005` に従い、Bare RN `15.1+`、Expo subset `16.4+`、arm64 device と arm64 Apple Silicon simulator とする。Intel `x86_64` simulator は formal support に含めない。device / simulator の同一 artifact grouping を release evidence で検証できる構成にする。将来 x86_64 simulator を追加する場合は、対応 slice、Intel host / runtime、CI および同等の lifecycle / security evidence を含む新しい正式判断を必要とする。
...
### 12.9 React Native における秘密情報のメモリフロー
...
### 12.11 スレッド、並行実行、状態を持たない Store 処理

RN adapter を v1 の concurrency boundary / serialization authority とする。ただし、その authority は一つの logical consumer context に閉じず、同一 process 内で RN から同じ C ABI / Rust Core に到達し得る全 runtime / module-registry domain を覆う process-wide RN binding coordination の配下に置く。複数の RN JS runtime、module registry、module instance および logical consumer context は許可できるが、各 context が独立して Core access を認可する構成や、Core / C ABI の global thread-safety を前提にした構成は採用しない。

責任階層は `process-wide RN binding coordination → runtime / module-registry scope → logical consumer context → RN adapter admission → existing public C ABI contract → Rust Wallet Core` とする。process-wide coordination は全 RN domain の Core / C ABI admission、shared native resource の initialization / shutdown barrier、cross-context ordering、stale completion rejection および process-wide resource ownership を管理する。runtime / module-registry scope は登録状態、runtime validity、結果 delivery、runtime-local teardown を管理し、logical consumer context は context 内 ordering、reentrancy および request lifecycle を管理する。いずれの下位 scope も process-wide coordinator を bypass せず、別 runtime の lifecycle や共有 resource を破棄しない。

RN runtime からの全 Core invocation は process-wide coordinator と context adapter の双方の admission を通過する。v1 では read / mutation、secret-capable operation および Store processing を含め、process-wide に同時 Core / C ABI 実行を許可せず、process-wide admission order を実行順とする。context 内 serialization は cross-context safety の代替ではない。Core または C ABI の reentrant / concurrent capability を RN integration の public contract とせず、Core は許可された一つの invocation に対して既存の stateless semantics を実行する。Application が current Store authority として replacement を適用する責任は変わらず、Binding は Store history、Profile state、password cache、unlocked state、decrypted secret または current Store を共有保持しない。

process-wide admission 順序を process-wide execution 順序として扱い、現在の operation が Core completion、output validation、temporary release および result delivery の可否判定を終えるまで、別 context を含む次の operation を Core へ開始しない。callback 中の public facade 再入、JS callback、UI re-entry、同一 adapter への recursive invoke および shutdown 中の新規 invoke を禁止する。process-wide initialization は single-flight または同等の一意な lifecycle とし、未初期化状態でいずれの runtime からも operation を開始しない。runtime-local shutdown / invalidation は当該 context の admission barrier、process-wide teardown は全 runtime registration と in-flight operation を対象にする coordinator の barrier として区別する。

runtime A の teardown、reload、module registry 再生成または A の cancellation は A の新規 admission と A への result delivery を停止するが、runtime B の利用可能な lifecycle を暗黙に破棄しない。A の completion は process-wide lifecycle と A の runtime / context lifecycle がともに有効な場合だけ delivery し、無効なら stale result として cleanup 後に破棄する。shared native resource の load / initialization / integrity failureなど process-wide に access safety を保証できない failure の場合だけ、coordinator が全 RN domain の新規 admission を止め、影響する operation を明示的に失敗させる。context-local conversion、Core operation error、cancellation および result delivery failureは原則として当該 context に閉じ込める。mutex、queue、executor、thread affinity、generation、single-flight primitive、memory ordering および具体的な teardown hook は下流へ委譲する。

authority の依存方向は上位 coordination から下位 adapter、C ABI、Core への一方向とし、context-local coordination と process-wide coordination が相互に完了を待つ構造を要求しない。teardown は invalidated context の callback / result delivery を待って共有 resource を解放する方式を前提にせず、operation completion と cleanup の barrier を process-wide coordinator が管理する。exact lock ordering、queue scheduling および callback mechanism は下流で定義する。
...
### 12.12 Native 成果物、サプライチェーン、package 境界
...
### 12.13 バージョン、アーキテクチャ、New Architecture、Expo の承認済み基準

RN-specific product policy は [`react-native-platform-baseline.md`](../decisions/react-native-platform-baseline.md) の `PD-RN-001〜PD-RN-007 = APPROVED` を現行 baseline とする。以下の Option / Rationale / Compatibility / Maintenance の比較は承認前の判断根拠を保存する **履歴的・非規範的な比較**であり、各表の `Approved baseline` が現在の Design 判断である。候補比較から現行 support status を再推論してはならない。現行 Node.js 22.x / 24.x policy と既存 Browser / native Node / WASM の保証は変更しない。
...
| Recommendation | New Architecture / JSI の安定実装を利用できる stable line に範囲を限定する。 |
| Approved baseline | `>= 0.86.x` を floor とし、stable `0.86.x` を compatibility line、stable `0.87.x` を primary validation line とする。`0.88.x` 以降、canary、nightly、`next` は正式なre-baselineまでunsupported。 |
...
| Maintenance impact | A を採用し、API 24 をcurrent floorとして維持する。 |
| Recommendation | current RN support floor と実利用端末の共通範囲を採用し、legacy compatibility を「念のため」に足さない。 |
| Approved baseline | `minSdk = 24`。`targetSdk` / `compileSdk` はrelease時点のofficial policyへ追従する。 |
...
| Maintenance impact | A を採用し、Bare RN とExpo subsetの異なるfloorをmatrixで管理する。 |
| Recommendation | RN integration と target application が共通に検証できる modern floor。 |
| Approved baseline | Bare RN `15.1+`。Expo SDK 57 subsetは`16.4+`。 |
...
| Recommendation | arm64-v8a と x86_64 にformal evidenceを集中し、legacy ABIを根拠なく追加しない。 |
| Approved baseline | `arm64-v8a` device + `x86_64` emulator。`armeabi-v7a`、`x86` その他のABIはunsupported。 |
...
| Recommendation | arm64 device / arm64 Apple Silicon simulatorにformal evidenceを集中する。 |
| Approved baseline | arm64 physical device + arm64 Apple Silicon simulator。Intel `x86_64` simulatorはunsupported。 |
...
| Recommendation | New Architecture mandatory。将来Legacyを追加する場合だけ、別の正式判断と追加evidenceを要求する。 |
| Approved baseline | New Architecture / TurboModule / Codegen / private JSIをmandatoryとし、Legacy Architecture / Bridgeはunsupported。async化は§12.3 / DDR-RN-004の条件付きfuture decisionとして別管理する。 |
...
| Recommendation | Bare RN、Development Build、Prebuild / CNGをformal native integrationとして扱い、Expo Goへfallbackしない。 |
| Approved baseline | Bare RN、Expo Development Build、Expo Prebuild / CNG、custom native module workflowをformal supportとする。Expo SDK 57 stable + RN 0.86.xをformal compatibility lineとし、Expo Go、canary / nightly、unlisted SDK / RN mismatchはunsupported。 |

### 12.14 セキュリティ上の攻撃面と対策の配置
...
- **Compatibility implications**: New Architecture / TurboModule / Codegen / private JSI を mandatory とし、stable RN `0.86.x` を compatibility line、`0.87.x` を primary validation line とする。Legacy Architecture / Bridge、`0.88.x` 以降、canary、nightly、`next` は formal support に含めない。将来範囲を拡張する場合は別の正式判断と追加 evidence を必要とする。
...
- **Compatibility implications**: `minSdk = 24`、`arm64-v8a` device および `x86_64` emulator を formal support とする。`armeabi-v7a`、`x86` その他の ABI は unsupported とし、将来追加する場合は build、test、artifact、release evidence を伴う別の正式判断を必要とする。
- **Deferred details**: AAR / jni layout、Gradle / CMake、filename、`targetSdk` / `compileSdk` policy、loader method。
...
- **Compatibility implications**: Bare RN は iOS `15.1+`、Expo subset は iOS `16.4+` とし、arm64 physical device と arm64 Apple Silicon simulator を formal support とする。Intel `x86_64` simulator は unsupported とし、将来追加する場合は build、test、artifact、release evidence を伴う別の正式判断を必要とする。
- **Deferred details**: XCFramework / static archive / pod boundary、Xcode setting、artifact filename、approved slice の検証方式、link flags。

#### DDR-RN-009: process-wide RN binding concurrency authority

- **Decision**: process-wide RN binding coordination を、同一 process 内の全 RN runtime / module registry / logical consumer context に対する Core invocation admission、serialization、cross-context ordering、reentrancy、shared initialization および process-wide teardown barrier の authority とする。runtime / module-registry scope と logical consumer context は各自の registration、local validity、local ordering および result delivery を管理するが、process-wide boundary を bypass しない。Core / C ABI の concurrent thread-safety を RN integration contract にしない。
- **Alternatives considered**: context ごとの独立 serialization、Core / C ABI の thread-safety を public integration にする、mutation だけを条件付きで serialize して read を無秩序に並列化する、Application ごとに任意の ordering を任せる、複数 runtime を拒否する。
- **Rationale**: 複数 RN runtime / module registry が存在しても concurrency、initialization、shutdown、stale completion および shared resource lifetime の責任を一意にし、Store mutation ordering、secret overlap、reentrancy および failure cleanup を予測可能にする。複数 context は process-wide boundary の下で分離し、local failure を不要に他 context へ伝播させない。
- **Security implications**: process-wide coordination は secret/session/key cache、Profile state、current Store または authorization authority ではない。queued secret を保持しない admission policy、stale result rejection、callback re-entry 禁止および Core の zeroize / fail-closed semantics を維持する。
- **Compatibility implications**: v1 は read-only を含めて process-wide に並列化せず、既存 public synchronous contract と同じ operation / error / binary semantics を保つ。runtime-local teardown は他 runtime の shared resource を破棄せず、Node / Browser / WASM routing は変更しない。将来の cross-context 並列化は明示的な Design / Specification change が必要である。
- **Deferred details**: coordinator の実装、queue / mutex / executor、thread affinity、single-flight、generation、cancellation、shutdown barrier、lock ordering および memory ordering。
...
本 section で確定したのは、RN の責務構造、共通 facade、runtime 分離、C ABI の内部再利用、sync policy、secret / buffer ownership の invariant、error category、fail-closed、thread / Store 境界および承認済み support baseline である。次の事項は Specification / Implementation / release verification に委譲する。
...
- TurboModule spec、Codegen schema、JSI object / HostObject、JNI / Swift / Objective-C++ signature、callback / exception ABI
...
- approved Android API / ABI および iOS version / architecture baseline を実現する Gradle / CMake / AAR / per-ABI filename、podspec / XCFramework / archive / slice verification、Metro / autolinking / prebuild integration
...
| DR-RN-002 | process-wide RN binding coordination が全 RN runtime / module registry / logical consumer context の Core / C ABI admission、cross-context ordering、shared lifecycle、stale completion および teardown barrier を一意に管理し、下位 scope は bypass しない | `architecture.md` §12.3、§12.7; `bindings.md` §12.11、DDR-RN-009; `security.md` §12.5 |
...
| DR-RN-005 | `PD-RN-001〜PD-RN-007` の承認済み version、OS、ABI / architecture、New Architecture および Expo baseline を canonical Design へ反映し、承認前の選択肢比較を履歴的・非規範的情報、Browser baseline と negative-evidence 後の async / support exclusion を別の未決定レーンとして区別する | `architecture.md` §12.4、§12.6、§12.7、§13; `bindings.md` §12.2、§12.6〜§12.7、§12.13、DDR-RN-001、DDR-RN-007〜DDR-RN-008、§12.17、§13 |
...
これらの資料で示される ecosystem の方向性は、承認前の候補比較における feasibility と rationale の参考に限る。現在の product policy は [`react-native-platform-baseline.md`](../decisions/react-native-platform-baseline.md) の `PD-RN-001〜PD-RN-007 = APPROVED` を normative input とし、外部資料だけから再推論しない。Browser baseline と、negative responsiveness evidence 後の async contract / operation-specific RN support exclusion は、この承認済み RN platform baseline とは別の未決定レーンとして扱う。
