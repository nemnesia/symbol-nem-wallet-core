...
要件本文、対応するコンセプト、公開された前段レビュー、ユーザー提供資料、`docs/design/` の設計判断、適用可能な承認済み資料を根拠とする。移行前の `docs/decisions/` は既存判断の確認に限って参照する。API、schema、algorithm、KDF、nonce、ライブラリ、DB、UI、実装手順は、既存要求の欠落した品質特性や制約を説明する場合を除き、要件の欠陥へ変換しない。
